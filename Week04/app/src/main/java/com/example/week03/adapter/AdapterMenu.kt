...
class AdapterMenu(private val listItemMenu : ArrayList<MenuItem>)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        when(viewType){
            VIEW_TYPE_ONE ->{
                var view:View = LayoutInflater.from(parent.context).inflate(R.layout.custom_menu_viewtype1,parent,false)
                return ViewHolder(view)
            }
            VIEW_TYPE_TWO ->{
                var view:View = LayoutInflater.from(parent.context).inflate(R.layout.custom_menu_viewtype2,parent,false)
                return ViewHolder(view)
            }
            VIEW_TYPE_THREE ->{
                var view:View = LayoutInflater.from(parent.context).inflate(R.layout.custom_menu_viewtype3,parent,false)
                return ViewHolder(view)
            }
            VIEW_TYPE_FOUR ->{
                var view:View = LayoutInflater.from(parent.context).inflate(R.layout.custom_menu_viewtype4,parent,false)
                return ViewHolder(view)
            }
            else -> {
                var view:View = LayoutInflater.from(parent.context).inflate(R.layout.custom_menu_viewtype4,parent,false)
